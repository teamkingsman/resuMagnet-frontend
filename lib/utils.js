...
export function getCroppedImageUrlFromUrl(url, width, height, x, y) {
  ...
  const publicIdWithExtension = uploadIndex !== -1 ? urlParts.slice(uploadIndex + 2).join('/') : null;
  
  // Remove the file extension
  const publicIdWithoutExtension = publicIdWithExtension?.split('.').slice(0, -1).join('.');
  
  // Sanitize the public ID by replacing invalid characters within each part
  const sanitizedPublicId = publicIdWithoutExtension?.replace(/[^a-zA-Z0-9_./-]/g, '_');
